...
class EditPublicationScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<EditPublicationScreen> createState() => _EditPublicationScreenState();
}
...
class _EditPublicationScreenState extends ConsumerState<EditPublicationScreen> {
  ...
  void initState() {
    ...
    _descController = TextEditingController(text: 'Cancha de pasto sintético de última generación, con iluminación LED y camarines. Ideal para partidos y entrenamientos.');
    _catController = TextEditingController(text: 'Deporte');
    _urlController = TextEditingController(text: 'https://ejemplo.com/cancha.jpg');
      ...
      builder: (context, constraints) {
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            ...
            title: const Text('Editar publicación', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18)),
            centerTitle: false,
          ),
      ...
      child: Column(
        children: [
          ...
          if (!isPublisher)
            ...
            _buildSidebarItem(context, Icons.bar_chart_outlined, 'Dashboard', () => context.go('/dashboard'), isActive: true),
  ...
  Widget _buildWebHeader(BuildContext context, String title) {
    return Container(
      ...
      child: Row(
        children: [
          IconButton(icon: const Icon(Icons.arrow_back_ios_new, color: Color(0xFF1E70CD), size: 20), onPressed: () => context.pop()),
          const SizedBox(width: 16),
          Text(title, style: const TextStyle(color: Color(0xFF1E70CD), fontSize: 24, fontWeight: FontWeight.w900)),
        ],
      ),
    );
  }
  ...
  Widget _buildForm() {
    return Column(
      ...
      children: [
        const Text('Editar publicación', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
