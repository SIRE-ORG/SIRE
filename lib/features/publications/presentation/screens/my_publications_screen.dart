...
class MyPublicationsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<MyPublicationsScreen> createState() => _MyPublicationsScreenState();
}
...
class _MyPublicationsScreenState extends ConsumerState<MyPublicationsScreen> {
  ...
  Widget _buildHeader(BuildContext context, bool isWeb) {
    return Container(
      padding: EdgeInsets.only(top: isWeb ? 40 : 60, left: 24, right: 24, bottom: 24),
      ...
      child: Row(
        ...
        children: [
          Row(
            children: [
              if (!isWeb) IconButton(icon: const Icon(Icons.arrow_back_ios_new, color: Colors.white, size: 20), onPressed: () => context.pop()),
              Text(
                'Mis publicaciones',
          ...
          if (isWeb)
            ElevatedButton.icon(
              ...
              label: const Text('Crear publicación', style: TextStyle(fontWeight: FontWeight.bold)),
              style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF1E70CD), foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 16)),
            ),
        ],
      ),
    );
  ...
  Widget _buildPublicationCard(Map<String, dynamic> pub, int index) {
    ...
    final statusBg = isActive ? const Color(0xFFE8F5E9) : const Color(0xFFF5F5F5);
    final statusText = isActive ? const Color(0xFF2E7D32) : const Color(0xFF757575);
            ...
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(pub['title'], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Color(0xFF1E293B))),
                    const SizedBox(height: 4),
                    Text(pub['subtitle'], style: const TextStyle(color: Colors.grey, fontSize: 12)),
                  ],
                ),
              ),
              Container(
                ...
                child: Text(pub['status'], style: TextStyle(color: statusText, fontSize: 12, fontWeight: FontWeight.bold)),
              ),
            ],
          ),
            ...
            children: [
              ...
              Expanded(
                child: OutlinedButton(
                  ...
                  child: Text(isActive ? 'Pausar' : 'Activar', style: const TextStyle(color: Colors.grey, fontSize: 12, fontWeight: FontWeight.bold)),
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
