...
├── flujo-cristobal.md      ← este documento
└── dudas/
    ├── emilia/             ← dudas de Emilia
    └── cristobal/          ← aquí dejas tus archivos de dudas
```

### Cómo usar la carpeta de dudas

Cuando tengas una pregunta sobre el contrato o algo que necesites definir con Daniel, puedes crear un archivo en `docs/dudas/cristobal/`:

```
docs/dudas/cristobal/disponibilidad.md
```
...
git config user.name "Cristóbal"
...
git checkout -b cristobal/endpoint-publications
...
# 6. Sube tu rama
git push origin cristobal/endpoint-publications
```

### Cómo nombrar tus ramas

El formato es siempre: `cristobal/[qué-estás-haciendo]`

```
cristobal/endpoint-publications
cristobal/endpoint-reservations
cristobal/modelo-disponibilidad
cristobal/integracion-resend
cristobal/auth-magic-link
cristobal/supabase-realtime
```
...
3. Base: `dev` ← Compare: `cristobal/endpoint-publications`
...
Los archivos de dudas que hayas creado en `docs/dudas/cristobal/` viajan en el mismo PR. No necesitas un PR separado para documentación.
...
> 🔴 **Bloqueado** en [endpoint o tarea] — necesito [qué cosa específica] de [Daniel/Emilia]. Dejé una duda en `docs/dudas/cristobal/[archivo].md`

**Ejemplo:**
> 🔴 **Bloqueado** en endpoint de disponibilidad — necesito que Daniel confirme si el `slotId` lo genera el backend o el frontend. Dejé el detalle en `docs/dudas/cristobal/disponibilidad.md`
