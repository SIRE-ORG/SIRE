...
| Autenticación y usuarios | Arquitectura de auth en Flutter con Riverpod | Pantallas de login y registro | JWT, estados de cuenta, magic link via Supabase Auth |
...
|---|---|---|
| `GUEST` | Creado desde formulario de reserva, sin contraseña | → `ACTIVE` al crear contraseña via magic link |
                            ...
                            └── [Toca publicación]
                                    └── Vista de detalle
                                            ...
                                            └── [Selecciona slot → "Reservar"]
                                                    ...
                                                    │                           "Crea tu contraseña"
                                                    │                           ├── Acepta → magic link al correo → ACTIVE
                                                    │                           └── Omite → se solicita al intentar navegar
                                                    └── ACTIVE
...
| 4 | Contraseña diferida: prompt no bloqueante post-reserva, interceptado al navegar si se omite | ✓ lógica de intercepción con Riverpod | ✓ pantalla y modal de creación de contraseña | ✓ magic link via Supabase Auth |
...
#### Página-05 · Prompt de creación de contraseña

**Responsable:** Emilia (diseño) · Daniel (lógica de intercepción con Riverpod) · Cristóbal (magic link via Supabase Auth)

**Contenido:**

- Confirmación de reserva exitosa
- Explicación del beneficio de crear contraseña
- Botón "Enviar link al correo" → Supabase Auth envía magic link
- Botón "Ahora no" → continúa como GUEST

**Comportamiento:** Si el usuario omite y luego intenta una acción que requiere estado ACTIVE, Riverpod intercepta la navegación y muestra este prompt como bottom sheet.
...
| Contraseña | Diferida: prompt no bloqueante post-reserva, magic link via Supabase Auth |
...
*Documento de trabajo interno — SIRE v2.0*# SIRE — Documento de Definición del Proyecto
